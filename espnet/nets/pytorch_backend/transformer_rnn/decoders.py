...
#from espnet.nets.pytorch_backend.transformer_rnn.decoder import Decoder
...
class Decoder(torch.nn.Module, ScorerInterface):
    ...
    def __init__(self, eunits, odim, dtype, dlayers, dunits, sos, eos, self_att, att, verbose=0,
        ...
        self.att = att
        self.ff = PositionwiseFeedForward(256, dunits, dropout)
        self.projection = torch.nn.Linear(dunits, 256)
    ...
    def rnn_forward(self, ey, z_list, c_list, z_prev, c_prev):
        if self.dtype == "lstm":
            ...
            for l in six.moves.range(1, self.dlayers):
                ...
                #z_list[l] = self.ff(z_list[l])
                #c_list[l] = self.ff(c_list[l])
        else:
    ...
    def forward(self, hs_pad, hlens, ys_pad, strm_idx=0, tgt_lang_ids=None):
        ...
        hlens = [hs_pad.size(1) for i in range(hs_pad.size(0))]
        # loop for an output sequence
        for i in six.moves.range(olength):
            att_c, att_w = self.att[att_idx](hs_pad, hlens, self.dropout_dec[0](z_list[0]), att_w)
            #z_proj = self.projection(z_list[0])
            #att_c = self.att(self.dropout_dec[0](z_proj).unsqueeze(1), hs_pad, hs_pad, None)
            #att_c = self.layer_norm(att_c.squeeze(1))
            #att_c = self.self_att(self.dropout_dec[0](z_proj).unsqueeze(1), hs_pad).squeeze(1)
        ...
        self.loss = self.labeldist(y_all.view(batch, olength, -1), ys_out_pad)
    ...
    def generate_forward(self, hs_pad, hlens, ys_pad, recog_args, strm_idx=0, topk=0, maxlenratio=1.0, minlenratio=0.3, rnnlm=None, tgt_lang_ids=None):
        """Decoder forward
        ...
        for i in six.moves.range(olength):
            att_c, att_w = self.att[att_idx](hs_pad, hlens, self.dropout_dec[0](z_list[0]), att_w)
            if i > 0 and random.random() < 1.0: #self.sampling_probability:
                z_out = self.output(z_all[-1])
                z_out = np.argmax(z_out.detach().cpu(), axis=1)
                z_out = self.dropout_emb(self.embed(to_device(self, z_out)))
                ey = torch.cat((z_out, att_c), dim=1)  # utt x (zdim + hdim)
...
def decoder_for(args, odim, sos, eos, self_att, att, labeldist):
    return Decoder(args.eunits, odim, args.dtype, args.dlayers, args.dunits, sos, eos, self_att, att, args.verbose,
