...
from espnet.nets.pytorch_backend.transformer.sid_decoder_layer import SID_DecoderLayer
...
class SID_Decoder(ScorerInterface, torch.nn.Module):
    ...
    def __init__(self, odim,
        ...
        if use_output_layer:
            self.embedding_layer = torch.nn.Linear(attention_dim, attention_dim)
    ...
    def forward(self, tgt, tgt_mask, memory, memory_mask):
        ...
        x, tgt_mask, memory, memory_mask = self.decoders(memory, memory_mask, memory, memory_mask)
        x = x.mean(dim=1)
    ...
    def recognize(self, tgt, tgt_mask, memory):
        ...
        """
        x = self.embed(tgt)
        x, tgt_mask, memory, memory_mask = self.decoders(memory, tgt_mask, memory, None)
        x = x.mean(dim=1)
        x = self.embedding_layer(x)
        if self.normalize_before:
    ...
    def inference(self, memory, memory_mask):
        ...
        """
        #x = self.embed(memory)
        #memory_mask = subsequent_mask(len(memory)).unsqueeze(0)
        x, tgt_mask, memory, memory_mask = self.decoders(memory, memory_mask, memory, None)
        x = x.mean(dim=1)
        x = self.embedding_layer(x)
        if self.normalize_before:
            x_ = self.after_norm(x)
        else:
            x_ = x[:, -1]
        if self.output_layer is not None:
            x_ = torch.log_softmax(self.output_layer(x_), dim=-1)
            return x_, x
        else:
            return x_, x
