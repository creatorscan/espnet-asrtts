...
class Decoder(torch.nn.Module, ScorerInterface):
    ...
    def __init__(self, eprojs, odim, dtype, dlayers, dunits, sos, eos, att, verbose=0,
        ...
        self.out_encoder = torch.nn.LSTM(dunits, dunits, dlayers)
        self.ignore_id = -1
        ...
        self.kl_loss = LabelSmoothingLoss(odim, self.ignore_id, lsm_weight, False)
    ...
    def forward(self, hs_pad, hlens, ys_pad, strm_idx=0, tgt_lang_ids=None, self_att=None):
        """Decoder forward
        ...
        c_out_list = eys.new_zeros(self.dlayers, batch, self.dunits)
        z_out_list = eys.new_zeros(self.dlayers, batch, self.dunits)
        out_embed = eys.new_zeros(olength, batch, self.dunits)
        ...
        for i in six.moves.range(olength):
            ...
            if i > 0 and random.random() < self.sampling_probability:
                ...
                #out_embed[i, :, :] = z_out
                #embed_out, _ = self.out_encoder(out_embed, (c_out_list, z_out_list))
