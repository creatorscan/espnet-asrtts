...
from espnet.nets.pytorch_backend.transformer.mask import non_subsequent_mask
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def target_mask(self, ys_in_pad, iteration):
        ys_mask = ys_in_pad != self.ignore_id
        if iteration > self.duration:
            self.ctxt = -int(round(iteration / self.duration))
        else:
            logging.info('Getting the standard context')
        logging.warn(str(self.ctxt) + " used for iteration " + str(iteration)) 
        m = non_subsequent_mask(ys_mask.size(-1), ctxt=self.ctxt, device=ys_mask.device).unsqueeze(0)
        return ys_mask.unsqueeze(-2) & m

    def valid_target_mask(self, ys_in_pad):
        ys_mask = ys_in_pad != self.ignore_id
        m = subsequent_mask(ys_mask.size(-1), device=ys_mask.device).unsqueeze(0)
        return ys_mask.unsqueeze(-2) & m

    def forward(self, xs_pad, ilens, ys_pad, iteration=2, asrtts=False):
        '''E2E forward
        ...
        if self.training:
            ys_mask = self.target_mask(ys_in_pad, iteration)
            #ys_mask = self.valid_target_mask(ys_in_pad)
        else:
            ys_mask = self.valid_target_mask(ys_in_pad)
        if asrtts:
            self.acc = None
            y_all, _ = self.decoder.generate_forward(ys_in_pad, ys_mask, hs_pad, hs_mask)
            y_list = []
            ...
            #logging.warn(fake_ys_out_pad[0,0:10])
            #logging.warn(ys_out_pad[0,0:10])
            loss_att = self.criterion(y_all, fake_ys_out_pad, asrtts=True).view(hs_pad.size(0), olength, -1).sum(2).sum(1)
            true_loss_att = self.criterion(y_all, ys_out_pad, asrtts=True).view(hs_pad.size(0), olength, -1).sum(2).sum(1)
            logging.warn(loss_att)
            logging.warn(true_loss_att)
        else:
            pred_pad, pred_mask = self.decoder(ys_in_pad, ys_mask, hs_pad, hs_mask)
            self.pred_pad = pred_pad

            # 4. compute attenttion loss
        ...
        if alpha == 0:
            ...
            if asrtts:
                ...
                loss_ctc_data = float(loss_ctc)
            loss_att_data = float(loss_att.sum())

        loss_data = float(self.loss.sum())
    ...
    def recognize(self, feat, recog_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                #ys_mask = non_subsequent_mask(i + 1, ctxt=self.ctxt).unsqueeze(0)
